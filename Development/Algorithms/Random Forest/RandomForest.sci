// Macro for Random Forest -- Scilab 
...
// Function to train a randomForest model with given 
// rate and iterations to converge
function ypred = randomForest(x, y, trees, sample, xtest)
	
	n 		= length(x(:,1));
	ypreds 	= [];
	for i = 1:trees
		index 	= round(1 + (n - 1).*rand(1,sample));
		xuse 	= x(index, :)
		yuse 	= y(index, :)
		[questions, flags] = decisionTreeFit(xuse, yuse)
		pred = decisionTreePredict(xtest, questions, flags)
		ypreds 	= [ypreds; pred]
		disp('Training tree : ' + string(i))
	end

	ypred = []
	numTrees = length(ypreds(1, :))
	for i = 1:numTrees
		ypred = [ypred, median(ypreds(:, i))]
	end
